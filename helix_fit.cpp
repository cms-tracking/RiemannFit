/*!
 *\author Roberto Ribatti
 *\date 2017-08-28

 *\warning multiple scattering treatment is still experimental
 *\warning points must be ordered from inner to outer layer
 *\warning maximum number of fittable point is stored in global variable max_n
*/
...
constexpr unsigned int max_n = 8; //!< In order to avoid use of dynamic memory
...
MatrixNd Scatter_cov_rad (const Matrix2xNd& p2D, const Vector4d& fast_fit)
{
    ...
    const double sig2 = sqr(0.015/p *sqr(X/cos_f_theta)
                            *(1+0.038*log(X/cos_f_theta)));
    for(u_int k=0; k<n; ++k)
    {
        for(u_int l=k; l<n; ++l)
        {
            ...
            {
                scatter_cov_rad(k,l) += (rad(k)-rad(i))
                    *(rad(l)-rad(i))*sig2/sqr(sin(theta));
...
inline Matrix2Nd cov_radtocart (const Matrix2xNd& p2D, const MatrixNd& cov_rad)
{
    ...
    for(u_int i=0; i<n; ++i)
    {
        for(u_int j=i; j<n; ++j)
        {
            cov_cart(  i,  j) = cov_rad(i,j) * p2D(1,i) * rad_inv(i)
                * p2D(1,j) * rad_inv(j);
            cov_cart(i+n,j+n) = cov_rad(i,j) * p2D(0,i) * rad_inv(i)
                * p2D(0,j) * rad_inv(j);
            cov_cart(  i,j+n) =-cov_rad(i,j) * p2D(1,i) * rad_inv(i)
                * p2D(0,j) * rad_inv(j);
            cov_cart(i+n,  j) =-cov_rad(i,j) * p2D(0,i) * rad_inv(i)
                * p2D(1,j) * rad_inv(j);
...
MatrixNd cov_carttorad (const Matrix2xNd& p2D, const Matrix2Nd& cov_cart)
{
    ...
    for(u_int i=0; i<n; ++i)
    {
        ...
        if (rad(i) < 1.e-4) cov_rad(i,i) = cov_cart(i,i);
        else
        {
            cov_rad(i,i) = rad_inv2(i) * ( cov_cart(i,i) * sqr(p2D(1,i))
                               ...
                               const Vector4d& fast_fit)
{
    ...
    {
        //!< in case you have (0,0) to avoid dividing by 0 radius
        if (rad(i) < 1.e-4) cov_rad(i,i) = cov_cart(i,i); //TO FIX
        else
        {
            ...
            cov_rad(i,i) =1./(1.+tan_c2) * (cov_cart(i,i)
                + cov_cart(i+n,i+n)*tan_c2 - 2*cov_cart(i,i+n) * tan_c);
        }
    }
    return cov_rad;
}
...
inline VectorNd Weight_circle (const Matrix2xNd& p2D,
                               const MatrixNd& cov_rad_inv)
{
...
inline VectorNd Weight_line (const ArrayNd& x_err2, const ArrayNd& y_err2,
...
inline int Charge (const Matrix2xNd& p2D, const Vector3d& par_uvr)
{
    return (  (p2D(0,1)-p2D(0,0)) * (par_uvr.y()-p2D(1,0))
            - (p2D(1,1)-p2D(1,0)) * (par_uvr.x()-p2D(0,0)) > 0) ? -1 : 1;
}
...
void par_uvrtopak (circle_fit& circle, const double& B,
                   const bool& error)
{
    Vector3d par_pak;
...
VectorNd X_err2 (const Matrix3Nd& V,  const circle_fit& circle,
                 const MatrixNx5d& J, const bool& error)
{
    VectorNd x_err2(n);
    for(u_int i=0; i<n; ++i)
    {
...
circle_fit Circle_fit(const Matrix2xNd& hits2D, const Matrix2Nd& hits_cov2D,
    ...
    if(scattering) A = X*G*X.transpose();
    else
    {
    ...
    circle_fit circle;
    circle.par << par_uvr_(0)*s_inv + h_(0),
                  par_uvr_(1)*s_inv + h_(1),
                  par_uvr_(2)*s_inv;
    ...
    if(error)
    {
        ...
        MatrixNd C[3][3]; //cov matrix of 3D transformed points
        {
            ...
            C[2][2] = 2.*( Vcs_[0][0]*Vcs_[0][0] + Vcs_[0][0]*Vcs_[0][1]
                        +  Vcs_[1][1]*Vcs_[1][0] + Vcs_[1][1]*Vcs_[1][1] )
                    + 4.*( Vcs_[0][0]*t00 + Vcs_[0][1]*t01
                        +  Vcs_[1][0]*t10 + Vcs_[1][1]*t11 );
        }
        ...
        for(u_int a=0; a<6; ++a)
        {
            ...
            for(u_int b=a; b<6; ++b)
            {
                ...
                if(l==k)
                {
                    ...
                    if(i==j) t1 = t0;
                    else t1 = 2. * D_[i][l] * s_v.col(l);
                }
                else
                {
                    t0 = D_[j][l] * s_v.col(k) + D_[j][k] * s_v.col(l);
                    if(i==j) t1= t0;
                    else t1 = D_[i][l] * s_v.col(k) + D_[i][k] * s_v.col(l);
                }

                if(i==j)  E(a,b) = 0. + s_v.col(i).transpose() * (t0 + t1);
                else E(a,b) = 0. + (s_v.col(i).transpose() * t0)
                        + (s_v.col(j).transpose() * t1);
                if(b!=a) E(b,a) = E(a,b);
            }
        }
        ...
        Matrix4d Cvc; //joint cov matrix of (v0,v1,v2,c)
        {
            ...
            Cvc(3,3) = (v.transpose() * C0 * v) + (C0.cwiseProduct(t0)).sum()
                + (r0.transpose() * t0 * r0);
        }
        ...
        {
            const double t = 1./h;
            J3 << -v2x2_inv,  0,  v(0)*sqr(v2x2_inv)*2.,  0,
                  0,  -v2x2_inv,  v(1)*sqr(v2x2_inv)*2.,  0,
...
line_fit Line_fit (const Matrix3xNd& hits, const Matrix3Nd& hits_cov,
                   const circle_fit& circle, const Vector4d& fast_fit,
    ...
    if(error)
    {
        ...
        Matrix<double, 2, 3> J; //Jacobian of (v,c) -> (cotan(theta)),Zip)
        {
            ...
            J << -t0,  v(0)*t1,  0,
                 -c*v(0)*t0*t2,  v0_2*c*t1*t2,  -sqrt_*t0;
        }
...
helix_fit Helix_fit (const Matrix3xNd& hits, const Matrix3Nd& hits_cov,
                     ...
                     const bool& scattering=false)
{
    ...
    circle_fit circle = Circle_fit(hits.block(0,0,2,n),
        hits_cov.block(0,0,2*n,2*n), fast_fit, error, scattering);
